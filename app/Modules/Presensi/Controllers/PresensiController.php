...
class PresensiController extends Controller
{
    ...
    public function presensi_jurnal(Request $request, Jurnal $jurnal)
    {
        ...
        //cek tabel presensi
        $data_siswa = Presensi::get_presensi_by_idjurnal($jurnal->id);
        // dd($data_siswa);

        if ($data_siswa->count() == 0) {
            //dd("sampe sini");

            $this->insert_presensi_by_idkelas($jurnal->id_kelas, $jurnal->id, get_semester('active_semester_id'));
        }

        $data['siswa']  = Presensi::get_presensi_by_idjurnal($jurnal->id);
        $data['jurnal'] = $jurnal;
